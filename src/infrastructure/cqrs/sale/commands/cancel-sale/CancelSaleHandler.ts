...
import { CancelSaleHandler as ApplicationCancelSaleHandler } from '../../../../../application/cqrs/sale/commands/cancel-sale/cancel-sale.handler';
...
export class CancelSaleHandler implements ICommandHandler<CancelSaleCommand> {
  private readonly appHandler: ApplicationCancelSaleHandler;

  constructor(
    @Inject(UNIT_OF_WORK) uow: TypeOrmUnitOfWork,
  ) {
    this.appHandler = new ApplicationCancelSaleHandler(new CancelSaleUseCase(uow));
  }
