...
import { IdentificationTypeDb } from './entities/enums/identification-type.db-enum';
...
async function main() {
  ...
  const consumerFinalData = {
    identificationType: IdentificationTypeDb.CONSUMIDOR_FINAL,
    identificationNumber: '9999999999999',
    names: 'CONSUMIDOR FINAL',
  };

  let consumerFinal = await customerRepo.findOne({ where: { identificationNumber: consumerFinalData.identificationNumber } });
  if (consumerFinal) {
